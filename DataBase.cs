	...
	public class DataBase : MonoBehaviour
	{
	    ...
	    private List<Item> inventoryBase = new List<Item>();
	    ...
	    { 
		    return effectsBase.Remove(data);
	    }
	    ...
	    private void Update()
	    {
		    foreach (var e in effectsBase)
		    {
			    e.Duraction -= Time.deltaTime;
			    if (e.Duraction <= 0)
			    {
				    effectsBase.Remove(e);
			    }
		    }
	    }
	    ...
	    {
		    return enemiesBase.Where(e => e.Type == type);
	    }
	    ...
	    public bool AddItem(Item item)
	    {
		    ...
		    if (c_INVENTORY_CAPACITY - inventoryBase.Count >= 0)
		    {
			    return true;
		    }
		    else return false;
	    ...
	    {
		    // тут конечно можно и >= 0, смотря как понимать заполнять до верху
		    var itemsInRange = items.Count();
		    if (c_INVENTORY_CAPACITY - inventoryBase.Count - itemsInRange >= 1)
		    {
			    inventoryBase.AddRange(items);
			    return true;
		    }
		    else return false;
	    }
